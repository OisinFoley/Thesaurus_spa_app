...
//go to router.post('/word/addSynonym'
//to see why this file exists, and to justify our approach to adding synonyms in our 'api.js' file
...
module.exports = function(router){
	...
	router.post('/word/addSynonym',function(req,res){		
		...
		if(req.body.baseWord == null || req.body.baseWord == '' || req.body.synonym == null || req.body.synonym == '' ) {			
			...
			console.log(JSON.stringify(req.body));
			// var test = "bbb,ccc,ddd";
			// test = test.split(',');
			var test = req.body.synonym;

				
				//if user tries to enter empty String, we will catch it here
				//eg - [a,,,b,,c,,,,,d] becomes [a,b,c,d]				
				synonyms = test.filter(Boolean);
														
				//the reason I need the other approach in 'api.js' is because different orders of the synonyms - 
				//	[bbb,ccc,ddd] , [ddd,ccc,bbb]
				//will be considered unique by the database, thus causing duplicates
				//also we can't insert data without giving an alias(we use 'test' below),
				//the problem is that the data is stored as follows:
				//	{ "_id" : ObjectId("59c3cfac96e01f1e3ed8da02"), 
				//		"baseWord" : "ddd", "synonyms" : 
				//			[ { "test" : [ "aaa", "bbb", "ccc" ] } ], "__v" : 0 }
				//so we don't want to see 'test' in our object for obvious reasons. 
				//we want something closer to this:
				//	{ "_id" : ObjectId("59c1b82098b434320eefb11d"), 
				//		"baseWord" : "dog", "synonyms" : 
				//			[ { "synonym" : "hound" }, { "synonym" : "mongrel" }, { "synonym" : "mutt" }
				//	which we achieve using the approach in 'api.js'
